...
export function isCorrect(question: Question, answer: string): boolean {
    const trimmed_answer: string = answer.trim();
    let ret: boolean;
    question.expected.toLowerCase() === trimmed_answer.toLowerCase() ?
        (ret = true)
    :   (ret = false);
    return ret;
}
...
export function isValid(question: Question, answer: string): boolean {
    ...
    const found: string | undefined = question.options.find(
        (option: string): boolean => answer === option,
    );
    let ret: boolean;
    found === undefined ? (ret = false) : (ret = true);
    return ret;
}
...
export function toShortForm(question: Question): string {
    return `${question.id}: `.concat(question.name.slice(0, 10));
}
...
export function toMarkdown(question: Question): string {
    ...
    if (question.type === "multiple_choice_question") {
        console.log("I'm multiple choice!");
...
export function duplicateQuestion(id: number, oldQuestion: Question): Question {
    return {...oldQuestion, id: id, name: `Copy of ${oldQuestion.name}`, published: false};
}
